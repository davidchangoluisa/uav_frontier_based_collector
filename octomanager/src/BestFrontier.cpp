...
namespace best_frontier
{
		...
		octomap::OcTree* octree,  point3d currentPosition, KeySet& Cells)
	{
		...
		for(int i = 0; i < candidates.size(); i++)
		{
			...
			double kGain = _params.kGain;
			InfGainVector[i] = kGain * unknownVolume * exp(- _params.lambda * tempDistance); 
		}
	...
	double BestFrontier::calcMIBox(const octomap::OcTree *octree, const point3d &sensorOrigin)
	{
		...
		double a = _params.boxInfGainSize;
		...
		int unknownNum {0};
		int allNum {0};
		for(double ix = minPoint.x(); ix < maxPoint.x(); ix += _params.resolution)
		{
			for(double iy = minPoint.y(); iy < maxPoint.y(); iy += _params.resolution)
			{
				for (double iz = minPoint.z(); iz < maxPoint.z(); iz += _params.resolution)
				{
					allNum++;
